...
\begin{abstract}
By repimplementing DCPT and tweaking its parameters for the ALPHA 21264 processor the prefetcher got a speedup of 9.2\% on a subset of the SPEC CPU2000 running on the M5 simulator. A key observation of DCPT is that it does not do anything when it does not find a match, this leads to no speedup in programs that has a random pattern. By adding a sequential prefetch fallback mechanism the speedup was increased to 10.1\% compared to no prefetcher.
\end{abstract}
...
\section{Previous Works} Over the years the schemes has grown to be more and
more sophisticated. The simple, yet elegant, scheme of predicting sequential
access has proved to be quite resilient, performance wise, to newcomers. By
simply exploiting the principle of spatial locality, whenever a new block is
accessed it also fetches the next block in memory. This naive approach works
well, but fails to recognize longer strides or alternating access patterns.
...
Prediction Tables, originally proposed by Grannaes, Jahre and Natvig in 2009. By
adding a fallback solution of sequential prefetching, we achieved a speedup of
x.

\section{Further Work}

\subsection{Sequential fallback distance}
The currently implemented fallback mechanism is as minimal aggressive
...
the increased aggresivity from the amount prefetched only caused a
decrease in performance \ref{fig:fallback}. Changing the prefetch distance
was on the other hand not explored and is a good candidate for further
profiling. It could also be worthwhile to try and determine a relation
between available delta history and the best distance to use.
