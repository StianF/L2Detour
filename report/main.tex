...
\begin{abstract}
By repimplementing DCPT and tweaking its parameters for the ALPHA 21264 processor the prefetcher got a speedup of 8.5\% in the M5 simulator. A key observation of DCPT is that it does not do anything when it does not find a match, this leads to no speedup in programs that has a random pattern. By adding a sequential prefetch fallback mechanism the speedup was increased to 10.1\% compared to no prefetcher.
\end{abstract}
