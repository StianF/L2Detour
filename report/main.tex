...
\begin{abstract}
By reimplementing DCPT and tweaking its parameters for the ALPHA 21264 processor the prefetcher got a speedup of 9.2\% on a subset of the SPEC CPU2000 running on the M5 simulator. A key observation of DCPT is that it does not do anything when it does not find a match. By adding a sequential prefetch fallback mechanism the speedup was increased to 10.5\% compared to no prefetcher.
\end{abstract}
