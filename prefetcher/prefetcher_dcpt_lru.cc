...
void prefetch_access(AccessStat stat)
{
		...
		if(!found){
			if(length < l-1){
				...
				begindelete = true;	
				length = 0;
				Tick lowest_time = rpttable[0].time;
				for(i = 0; i < l; i++){
					if(rpttable[i].time < lowest_time){
						lowest_time = rpttable[i].time;
						length = i+l;
					}
				}
			}
